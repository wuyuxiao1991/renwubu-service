...
public class ZhuanwuLeaderRegistrationHandler implements ExcelHandler {
    ...
    public String getExcelType() {
        return ExcelTypeEnum.ZHUANWU_LEADER_REGISTRATION.name();
    }
    ...
    public void upload(MultipartFile file, UploadExcelRequest request) {
        ...
        for (int i = 4; i < rowNum; i++) {
            ...
            insertZhuanwuLeaderRegistration(row, request.getIdentity());
        }
    }
    ...
    private void fillRows(ZhuanwuLeaderRegistration zhuanwuLeaderRegistration, Sheet sheet, int i) {
        ...
        row.createCell(0).setCellValue(zhuanwuLeaderRegistration.id);
